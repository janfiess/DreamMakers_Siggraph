...
struct VertexOutputForwardBase
{
	...
	// next ones would not fit into SM2.0 limits, but they are always for SM3.0+
		...
		#endif
	#endif
	#if defined(_REFRACTION) || DECAL_PIERCEABLE || (defined(LOD_FADE_CROSSFADE) && !defined(_ALPHABLEND_ON) && !defined(_ALPHAPREMULTIPLY_ON))
...
)
{
		...
		half3 ditherScreenPos = i.screenPos.xyw;
...
struct VertexOutputForwardAdd
{
	...
	#if DECAL_PIERCEABLE || (defined(LOD_FADE_CROSSFADE) && !defined(_ALPHABLEND_ON) && !defined(_ALPHAPREMULTIPLY_ON))
		float4 screenPos				: TEXCOORD9;
	#endif
...
//#endif
	...
	// push lighting coords
		...
		UnityLight light = AdditiveLight (lightDir, LIGHT_ATTENUATION(i));
	#else
...
//			#endif
			UnityLight light = AdditiveLight (lightDir, latten);
		#else
			//
...
)
{
		...
		half3 ditherScreenPos = i.screenUV.xyw;
		ditherScreenPos.xy *= _ScreenParams.xy * 0.25;
		half2 projUV = ditherScreenPos.xy / ditherScreenPos.z;
		projUV.y = frac(projUV.y) * 0.0625 /* 1/16 */ + unity_LODFade.y; // quantized lod fade by 16 levels
		clip(tex2D(_DitherMaskLOD2D, projUV).a - 0.5);
