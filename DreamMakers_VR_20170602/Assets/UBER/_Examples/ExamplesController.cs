...
public class ExamplesController : MonoBehaviour {
	...
	public void ClickedArrow(bool rightFlag) {
		...
		mouseOrbitController.targetFocus = objectsParams[currentTargetIndex].target.transform.FindChild ("Focus");
	...
	public void Update() {
		...
		if (objectsParams[currentTargetIndex].Title=="Ice block" && Input.GetKeyDown (KeyCode.L)) {
			GameObject go=objectsParams[currentTargetIndex].target.transform.FindChild("Amber").gameObject;
